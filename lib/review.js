...
export function getPublicErrorMessage(error) {
  ...
  if (message.includes('ETIMEDOUT') || message.includes('GEMINI_REQUEST_TIMEOUT')) return '连接 Gemini 超时，请检查 GEMINI_API_BASE_URL 是否可访问';
  if (message.includes('fetch failed')) return '无法连接 Gemini API，请检查网络或 GEMINI_API_BASE_URL 配置';
...
请完成：识别主要文字；判断整体书写情况；从笔画、结构、章法、墨色、书体特征点评；找出 3-5 个最值得标注的点评点；如果图片模糊或遮挡，也给出可识别范围内点评并提示重新上传更清晰图片；不要编造无法判断的内容。

话术风格（很重要）：
- 像真人老师在微信里打字点评，口语自然、温和鼓励，不要像作文或报告
- 少用双引号、书名号、破折号等符号；提到某个字时直接写，例如：成字的撇可以再有力一点
- 不要写「成」字、"成"字 这种带引号的写法
- title、comment、suggestion、summary、user_friendly_feedback 都要口语化
- suggestion 字段不要以「建议：」开头，直接说怎么练
- 专业判断可以保留，但表达要像人敲键盘写出来的，不要模板腔
...
- target_text 必须写该框对应的具体汉字，不要写整体、章法、墨色等笼统描述
...
function normalizeReview(review, styleLabel) {
  ...
  return {
    ...
    overall_comment: {
      summary: humanizeReviewText(review.overall_comment?.summary || '整体书写比较认真，字形和排布已有基础，后续可继续加强结构稳定性。'),
      strengths: normalizeStringList(review.overall_comment?.strengths, ['态度认真，整体排布较整齐']).map(humanizeReviewText),
      main_problems: normalizeStringList(review.overall_comment?.main_problems, ['个别字的重心和笔画间距还可以更稳定']).map(humanizeReviewText),
      next_focus: normalizeStringList(review.overall_comment?.next_focus, ['练习重心', '练习横画间距']).map(humanizeReviewText)
    },
    annotations: annotations.slice(0, 6).map((item, index) => normalizeAnnotation(item, index)),
    practice_advice: {
      today_focus: humanizeReviewText(review.practice_advice?.today_focus || '今天建议重点练习字的重心和横画间距。'),
      practice_method: humanizeReviewText(review.practice_advice?.practice_method || '选择 3 个常用字，先观察中轴线，再每个字连续练习 5 遍。'),
      estimated_practice_time: String(review.practice_advice?.estimated_practice_time || '15 分钟')
    },
    user_friendly_feedback: humanizeReviewText(review.user_friendly_feedback || '这次作业完成得很认真，已经能看出你在控制字形结构。接下来把重心写稳，进步会更明显。')
  };
}
...
function normalizeAnnotation(item, index) {
  ...
  return {
    ...
    bbox,
    drawable,
    title: humanizeReviewText(item?.title || '结构可再调整'),
    comment: humanizeReviewText(item?.comment || '这个位置可以继续观察笔画之间的关系。'),
    suggestion: humanizeReviewText(item?.suggestion || '下次书写前先慢慢观察位置，再落笔练习。'),
    severity: ['low', 'medium', 'high'].includes(item?.severity) ? item.severity : 'medium',
    display_style: drawable ? 'box' : 'text_only'
  };
}

function humanizeReviewText(value) {
  return String(value || '')
    .replace(/^建议[：:]\s*/u, '')
    .replace(/[「」『』“”"']/gu, '')
    .replace(/\s{2,}/g, ' ')
